...
PROJECT_CONFIG: dict[str, dict[str, str | list[str]]] = {
    ...
    "project-zahlen": {
        ...
        "desc": "Custom high-performance game engine and rendering framework built from scratch with modern C++ and Vulkan.",
        "tech": ["C++23", "Vulkan", "Graphics", "Game Engine"],
    },
}
...
def main():
    ...
    prs_search = get_safe(
        session,
        f"https://api.github.com/search/issues?q=author:{USERNAME}+type:pr+is:merged+-user:{USERNAME}&per_page=5",
        {"items": []},
    )
    repos = get_safe(
        session, f"https://api.github.com/users/{USERNAME}/repos?per_page=100", []
    )
    ...
    data: GitHubData = {
        ...
        "stars": {
            r.get("name", "").lower(): r.get("stargazers_count", 0) for r in repos
        },
